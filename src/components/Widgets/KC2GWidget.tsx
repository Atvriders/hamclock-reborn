...
const KC2GWidget: React.FC = () => {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);
  const [cacheBust, setCacheBust] = useState(Date.now());
  const [expanded, setExpanded] = useState(false);

  const imageUrl = `${KC2G_URL}?t=${cacheBust}`;
  ...
  return (
    <div style={{
      ...
      <div
        ...
        title="KC2G MUF Map — click to expand"
      >
        {loading && !error && (
      ...
      <div style={{ fontSize: 8, color: '#4a5568', marginTop: 3 }}>Source: prop.kc2g.com</div>
      ...
      {expanded && (
        ...
        >
          <div style={{ position: 'relative' }}>
            ...
            <img src={imageUrl} alt="KC2G MUF Map (expanded)" style={{ maxWidth: '90vw', maxHeight: '85vh', border: '2px solid #1a2332', borderRadius: 8 }} />
          </div>
        </div>
      )}
    </div>
